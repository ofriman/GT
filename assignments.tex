...
\begin{proof}
Assume $M$ is irreducible, therefore all states are pairwise accessible from one another  \cite{classifying_markov}. Let $x,y \in \Omega$ and $y$ is accessible from $x$ from above \ref{problem5.1} we know that $(P + P^2 + \cdots + P^{\abs{\Omega}})_{xy} > 0$ and so the claim hold for this direction.
...
\begin{proof}
State $3$ and $4$ are not accessible from $1$ and $2$.
\begin{equation*}
(I + P + P^2 + \cdots + P^{\abs{\Omega}})_{\{1,2\},\{3,4\}} =\sum_{j = 1}^\Omega = P^j_{\{1,2\},\{3,4,\}} = 0.
\end{equation*}
State 2 is accessible from state 1 and state 1 is accessible from state 2
\begin{equation*}
P^1_{1,2} = \frac{1}{2} \text{, } P^1_{2, 1} = \frac{1}{2}
\end{equation*}
State 3 is not accessible from state 4
\begin{equation*}
(I + P + P^2 + \cdots + P^{\abs{\Omega}})_{4,3} =\sum_{j = 1}^\Omega = P^j_{4,3} = 0.
\end{equation*}
...
P^1_{6,4} =0.3
\end{equation*}
Let $x \in \{1,2,4,5,6\}$ then
\begin{equation*}
r_x = Pr[M\text{ revisits }x \mid X_0 = x] =1
\end{equation*}
$Pr[N_3 > 1] = 0$ and so this state is transient.
So, [1,2] recurrent class
[3] transient class

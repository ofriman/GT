...
\end{proof}
 \item
  \begin{proof}
	...
	{\color{red} no idea, maybe same approach as above}
...
\end{problem}
\begin{proof}
This is follow from Tanner.
\begin{quote}{\cite[Lemma 15]{expander}}{
(\cite[(Tanner]{tanner}) Let $G$ be a connected $d$-regular graph on $n$ vertices. If $\lambda(G) \neq d$ then
 \begin{equation*}
\abs{N_G(X)} \geq \left(\frac{d^2}{\lambda(G)^2 + (d^2 - \lambda(G)^2)\frac{\abs{X}}{n} }\right)\abs{X}
\end{equation*}
}\end{quote}

Let use Tanner inequity and Set $\abs{X} = \frac{n}{2}$

 \begin{align*}
\abs{N_G(X)} & \geq \left(\frac{d^2}{\lambda(G)^2 + (d^2 - \lambda(G)^2)\frac{1}{2} }\right)\frac{n}{2} \\&=
\left(\frac{2 d^2}{d^2 + \lambda(G)^2}\right)\frac{n}{2}
\end{align*}
\end{proof}		
